...
data_raw <- data_raw |> select(-(file:Liga)) |> unnest_longer(Data) |> unpack(Data)
...
team_kurz <- c("Angels", "Dragons", "P.Eagles", "Giants", "Indians", "Raiders", "Spartans", "Steelsharks",
               "Badgers", "Bastards/Hawks", "C.Eagles", "Invaders", "Knights", "Legionaries", "Vikings", "Vipers")
# teams <- unique(c(data_raw$Heim, data_raw$Gast))
teams <- data_raw |> select(Saison, Stufe, Div, Heim) |> unique() |> rename(Team = Heim)
